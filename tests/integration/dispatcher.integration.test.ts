...
test('ProjectDispatcher - ACTIVE: executes ClosedLoopEngine and invokes generic runtime', async (t) => {
  ...
  assert.ok(runtimeCallCount > 0);
  assert.equal(result.context?.projectId, 'proj-active');
  assert.equal(result.context?.workspacePath, repoDir);
});

test('ProjectDispatcher - PAUSED: blocks before WorkspaceResolver, SafetyGate, Engine, and runtime (RUNTIME_CALL_COUNT = 0)', async (t) => {
  ...
  assert.equal(runtimeCallCount, 0, 'Runtime must NEVER be called when project context is PAUSED');
});

test('ProjectDispatcher - ARCHIVED: blocks before WorkspaceResolver, SafetyGate, Engine, and runtime (RUNTIME_CALL_COUNT = 0)', async (t) => {
  ...
  assert.equal(runtimeCallCount, 0, 'Runtime must NEVER be called when project context is ARCHIVED');
});
...
test('ProjectDispatcher - UNINITIALIZED: automatically bootstraps context and proceeds to execution', async (t) => {
  ...
  const engineFactory = (ctx: ExecutionContext) => {
    engineRunCount++;
    return new ClosedLoopEngine(undefined, createTestRuntime(() => runtimeCallCount++), { executionContext: ctx });
  };
...
test('ProjectDispatcher - PROJECT/WORKSPACE MISMATCH: SafetyGate blocks execution with RUNTIME_CALL_COUNT = 0', async (t) => {
  ...
  assert.equal(runtimeCallCount, 0, 'Runtime must NEVER be invoked when safety gate blocks execution');
});
